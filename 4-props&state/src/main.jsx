...
class Clock extends React.Component {
  ...
  render(){
return (<div>
<p>it is Clock Iran :<span className='font-bold text-blue-600'>{this.state.date.toLocaleTimeString()}</span></p>

<p>it is Clock Uae :<span className='font-bold text-blue-600'>{this.state.date.toLocaleTimeString()}</span></p>
</div>
)
  }
}
...
<h2 className='p-2 bg-black  text-white w-1/3 mx-auto'  ><span className='text-red-700 font-bold text-3xl '>{count}</span> back in count</h2>
<button className='bg-sky-400 m-2 p-3 border w-1/3 mx-auto border-black rounded-2xl hover:bg-sky-600' onClick={()=>setCount(count-1)}>count Down</button>
