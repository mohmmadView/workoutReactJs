...
let [id , setId]=useState(0);
    return(
     <div className="w-10/12 mx-auto my-12 bg-base-300 flex p-10 text-xl">
        ...
        <Prism language="js" code={codeChatList} />
     </div>
    )
}
const contacts = [
...
return(
    <div className="w-5/12">
      {list.map(contact => {
        return <button onClick={()=>giveId(contact.id)}  className="w-11/12 m-1 btn btn-primary flex" key={contact.id} >{contact.name}</button>})}
    </div>
)
}
 ...
 let  [message ,setMessage]=useState('');
    return(
        <div className="w-7/12 flex flex-col">
            <textarea  className="w-full textarea textarea-bordered h-4/6"
