const DataText = [
  ...
  },
  {
    ChoosingTheStateFa: () => {
      return (
        <p className="rtl" style={{ direction: "rtl" }}>
        ساختاربندی خوب می‌تواند بین مؤلفه‌ای که برای اصلاح و اشکال‌زدایی دلپذیر است و مؤلفه‌ای که منبع ثابتی از اشکال است
         تفاوت ایجاد کند. مهمترین اصل این است که وضعیت نباید حاوی اطلاعات اضافی یا تکراری باشد.
          اگر حالت غیر ضروری وجود داشته باشد، به راحتی می توان فراموش کرد که آن را به روز کنید و اشکالات را معرفی کنید!
        </p>
      );
    },
  },
  ...
  },
  {
  ...
  },
   {
    PreservingAndResettingFa: () => {
      return (
        <p>
          وقتی یک کامپوننت را دوباره رندر می‌کنید، React باید تصمیم بگیرد که کدام قسمت‌های درخت را نگه دارد (و به‌روزرسانی کند)، و کدام قسمت‌ها را دور انداخته یا از ابتدا دوباره ایجاد کند. در بیشتر موارد،
           رفتار خودکار React به اندازه کافی خوب عمل می کند. به‌طور پیش‌فرض، React بخش‌هایی از درخت را حفظ می‌کند که با درخت مؤلفه‌ای که قبلاً رندر شده است، مطابقت دارند.
            با این حال، گاهی اوقات این چیزی نیست که شما می خواهید. در این برنامه چت، تایپ یک پیام و سپس تغییر
            گیرنده، ورودی را بازنشانی نمی کند. این می تواند باعث شود کاربر به طور تصادفی پیامی را به شخص اشتباهی ارسال کند:
        </p>
      );
    },
  },
  ...
  },
  {
  ...
  },
   {
    WhatIsReactContextFa: () => {
      return (
        <p>
         React Context راهی را در اختیار ما قرار می دهد تا داده ها 
         را از طریق درخت کامپوننت به جایی که به آن نیاز داریم منتقل کنیم، بدون اینکه نیازی به ارسال دستی 
         props در هر سطح باشد. این به عنوان یک فضای ذخیره سازی جهانی برای تمام اجزای شما در پروژه شما عمل می کند.
        </p>
      );
    },
  },
  {
  ...
  },
   {
    ReactContextDifferentFa: () => {
      return (
        <p>
          در prop threading، داده ها از مولفه والد به جزء فرزند منتقل می شود. اگر فرزند آن مؤلفه به همان پایه نیاز داشته باشد، تا زمانی که مؤلفه مورد نیاز
           داده را دریافت کند، ارسال می شود. در حالی که ساده است، می تواند در هنگام عبور از 
          ساختارهای تو در تو پیچیده شود و این منجر به حفاری پایه می شود. در مقابل، React Context اجازه می‌دهد تا داده‌ها را از مؤلفه والد به هر مؤلفه
           تودرتویی که به آن نیاز دارد منتقل شود 
          و فرآیند را ساده‌تر کند..
        </p>
      );
    },
  },
